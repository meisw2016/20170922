...
@ServerEndpoint(value = "/websocket/mysql/{params}")
@Component
public class WebSocketMySQL {

	private static Log logger = LogFactory.getLog(WebSocketMySQL.class);

	private static HostMapper hostMapper;
	
	@Autowired
	public void setHostMapper(HostMapper hostMapper) {
		WebSocketMySQL.hostMapper = hostMapper;
	}
	...
	private static CopyOnWriteArraySet<WebSocketMySQL> webSocketSSHServerSet = new CopyOnWriteArraySet<WebSocketMySQL>();
	...
	public void onOpen(Session websocketSession, @PathParam("params") String params) {
		...
		if (StringUtils.isNotEmpty(hostIp) && hostIp != "192.168.251.151") {
			...
			case "install":
				...
				this.client.write("sh addons.sh install mysql\r");
				break;
			case "uninstall":
				this.client.write("cd yusp-autoinstall/yusp-autoinstall-shell\r");
				this.client.write("sh addons.sh uninstall mysql\r");
				break;
			case "start":
				this.client.write("service mysql start\r");
				break;
			case "stop":
				this.client.write("service mysql stop\r");
				break;
			case "status":
				this.client.write("service mysql status\r");
				break;
			default:
				break;
			}
		} else {
			throw new RuntimeException("ip地址不能为空或此IP为测试服务器，不能进行mysql相关操作，请更换ip后再试");
		}
	}
