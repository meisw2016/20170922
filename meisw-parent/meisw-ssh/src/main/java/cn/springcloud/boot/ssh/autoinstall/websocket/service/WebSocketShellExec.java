...
@ServerEndpoint(value = "/websocket/shell/{params}")
@Component
public class WebSocketShellExec {

	private static Log logger = LogFactory.getLog(WebSocketShellExec.class);

private static HostMapper hostMapper;
	
	@Autowired
	public void setHostMapper(HostMapper hostMapper) {
		WebSocketShellExec.hostMapper = hostMapper;
	}
	...
	private static CopyOnWriteArraySet<WebSocketShellExec> webSocketSSHServerSet = new CopyOnWriteArraySet<WebSocketShellExec>();
	...
	public void onOpen(Session websocketSession, @PathParam("params") String params) {
		...
		if (StringUtils.isNotEmpty(hostIp)) {
			...
			// 执行一些命令
			this.client.write("cd yusp-autoinstall\r");
			this.client.write("cat yusp.conf\r");
			this.client.write("cd tools\r");
			this.client.write("./YUSP status\r");
		} else {
	...
	public static synchronized void addOnlineCount() {
		WebSocketShellExec.onlineCount++;
	}
