...
@ServerEndpoint(value="/websocket/config/{params}")
@Component
public class WebSocketYUSPConfigService {
	
	private static Log logger = LogFactory.getLog(WebSocketYUSPConfigService.class);
	
	private static HostMapper hostMapper;
	
	@Autowired
	public void setHostMapper(HostMapper hostMapper) {
		WebSocketYUSPConfigService.hostMapper = hostMapper;
	}
    ...
    private static CopyOnWriteArraySet<WebSocketYUSPConfigService> webSocketSSHServerSet = new CopyOnWriteArraySet<WebSocketYUSPConfigService>();
    ...
    public void onOpen(Session websocketSession,@PathParam("params") String params) {
    	...
    	if (StringUtils.isNotEmpty(hostIp)) {
            ...
            switch (operation) {
            case "create":
            	logger.info("config WebSocket 已经建立连接...");
				break;
			default:
    ...
    public static synchronized void addOnlineCount() {
    	WebSocketYUSPConfigService.onlineCount++;
    }
