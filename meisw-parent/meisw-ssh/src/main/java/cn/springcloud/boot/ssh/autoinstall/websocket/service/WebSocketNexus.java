...
@ServerEndpoint(value = "/websocket/nexus/{params}")
@Component
public class WebSocketNexus {

	private static Log logger = LogFactory.getLog(WebSocketNexus.class);

	private static HostMapper hostMapper;
	
	@Autowired
	public void setHostMapper(HostMapper hostMapper) {
		WebSocketNexus.hostMapper = hostMapper;
	}
	...
	private static CopyOnWriteArraySet<WebSocketNexus> webSocketSSHServerSet = new CopyOnWriteArraySet<WebSocketNexus>();
	...
	public void onOpen(Session websocketSession, @PathParam("params") String params) {
		...
		if (StringUtils.isNotEmpty(hostIp) && hostIp != "192.168.251.151") {
			...
			case "install":
				...
				this.client.write("sh addons.sh install nexus\r");
				break;
			case "uninstall":
				this.client.write("cd yusp-autoinstall/yusp-autoinstall-shell\r");
				this.client.write("sh addons.sh uninstall nexus\r");
				break;
			case "start":
				this.client.write("service nexus start\r");
				break;
			case "stop":
				this.client.write("service nexus stop\r");
				break;
			case "status":
				this.client.write("service nexus status\r");
				break;
			default:
				break;
			}
		} else {
			throw new RuntimeException("ip地址不能为空或此IP为测试服务器，不能进行nexus相关操作，请更换ip后再试");
		}
	}
