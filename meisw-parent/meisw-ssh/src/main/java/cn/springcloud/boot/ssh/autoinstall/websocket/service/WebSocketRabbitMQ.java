...
@ServerEndpoint(value = "/websocket/rabbitmq/{params}")
@Component
public class WebSocketRabbitMQ {

	private static Log logger = LogFactory.getLog(WebSocketRabbitMQ.class);

	private static HostMapper hostMapper;
	
	@Autowired
	public void setHostMapper(HostMapper hostMapper) {
		WebSocketRabbitMQ.hostMapper = hostMapper;
	}
	...
	private static CopyOnWriteArraySet<WebSocketRabbitMQ> webSocketSSHServerSet = new CopyOnWriteArraySet<WebSocketRabbitMQ>();
	...
	public void onOpen(Session websocketSession, @PathParam("params") String params) {
		...
		if (StringUtils.isNotEmpty(hostIp)) {
			...
			if (host == null) {
				throw new RuntimeException("ip不正确");
			}
			...
			case "install":
				...
				this.client.write("sh addons.sh install rabbitmq\r");
				break;
			case "uninstall":
				this.client.write("cd yusp-autoinstall/yusp-autoinstall-shell\r");
				this.client.write("sh addons.sh uninstall rabbitmq\r");
				break;
			case "start":
				this.client.write("service rabbitmq-server start\r");
				break;
			case "stop":
				this.client.write("service rabbitmq-server stop\r");
				break;
			case "status":
				this.client.write("service rabbitmq-server status\r");
				break;
			default:
				break;
			}
