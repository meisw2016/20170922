...
@ServerEndpoint(value = "/websocket/nginx/{params}")
@Component
public class WebSocketNginx {

	private static Log logger = LogFactory.getLog(WebSocketNginx.class);

	private static HostMapper hostMapper;
	
	@Autowired
	public void setHostMapper(HostMapper hostMapper) {
		WebSocketNginx.hostMapper = hostMapper;
	}
	...
	private static CopyOnWriteArraySet<WebSocketNginx> webSocketSSHServerSet = new CopyOnWriteArraySet<WebSocketNginx>();
	...
	public void onOpen(Session websocketSession, @PathParam("params") String params) {
		...
		if (StringUtils.isNotEmpty(hostIp) && hostIp != "192.168.251.151") {
			...
			case "install":
				...
				this.client.write("sh addons.sh install nginx\r");
				break;
			case "uninstall":
				this.client.write("cd yusp-autoinstall/yusp-autoinstall-shell\r");
				this.client.write("sh addons.sh uninstall nginx\r");
				break;
			case "start":
				this.client.write("service nginx start\r");
				break;
			case "stop":
				this.client.write("service nginx stop\r");
				break;
			case "status":
				this.client.write("service nginx status\r");
				break;
			default:
				break;
			}
		} else {
			throw new RuntimeException("ip地址不能为空或此IP为测试服务器，不能进行nginx相关操作，请更换ip后再试");
		}
	}
